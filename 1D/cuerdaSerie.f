...
c     centradas
      ...
      if (or.eq.2)then
         call primerOrden(pendiente,intervalos,mat,h,ht,n)
      end if
...
c     subrutunas depara el caso de primer y segundo ordern
      ...
      subroutine primerOrden(pendiente,intervalos,mat,h,ht,n)
      implicitnone
      real*8::t,x,pendiente,h,ht,e,c,fmult,fsum
      ...
      if (i.eq.1)then
        ...
        e=(mat(i,j-1)-2*mat(i,j)+mat(i,j+1))*c
        mat(i+1,j)=2*mat(i,j)-mat(i-1,j)+e+Fsum(t,x)*(ht**2)
