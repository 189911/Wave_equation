...
!     encias finitas
      real*8:: h,ht,L,A,FinX,FinT,x,e,c,r,pendiente,Fmult,Fsum,t,y,corX1
      real*8:: corY1,corZ1,c2,r2,corX2,corY2,corZ2,max
      integer::j,i,k,intervalos,or,nx,p,ny,u,s,N_Mitad1,N_Mitad2,M1,M2
      character*100::z
      real*8,dimension(1000,200,200)::mat
      real*8,dimension(1,500,500)::Mitad1,Mitad2,Mitad3,mitad4
...
c     prociedemos a iniciar las primeras condiciones de la matriz
      ...
      write(*,*)"Sistema de cuatro modos de vibracion "
...
c     vamos hacer I como el contador de T, J oara X y u para Y
      do i=1,intervalos
      do j=1,nx
      do u=1,ny
      mat(i,j,u)=0.0
...
c     empezamos nuestras condiciones inicales----------------------
      ...
      !condiciones para los calores de frontera de x
      do i=1,intervalos
      do u=1,ny
      mat(i,1,u)=0.0
      mat(i,nx,u)=0.0
      ...
      !condiciones para los calores de frontera para t=0
      do j=1,nx
      do u=1,ny
      mat(1,j,u)=0.0
      mat(1,j,u)=0.0
      ...
      !! inicialisamos las condiciones de forntera de las matrices 1, 2
      ...
      call Piramide(Mitad1,N_Mitad1,M1,h*N_Mitad1,L,corX1,corY1,corZ1,h)
      call Piramide(Mitad2,N_Mitad2,M1,h*N_Mitad2,L,corX2,corY2,corZ2,h)
      call Piramide(Mitad3,N_Mitad1,M2,h*N_Mitad1,L,corX1,corY1,corZ2,h)
      call Piramide(Mitad4,N_Mitad2,M2,h*N_Mitad2,L,corX2,corY2,corZ1,h)
      do j=1,nx
      do u=1,ny
      if (j.Le.N_Mitad1)then
      if (u.le.M1)THEN
      mat(1,j,u)=Mitad1(1,j,u)
      else
      mat(1,j,u)=Mitad3(1,j,u-M1)
      end if
      ELSE

      if (u.le.M1)THEN
      mat(1,j,u)=Mitad2(1,j-N_Mitad1,u)
      else
      mat(1,j,u)=Mitad4(1,j-N_Mitad1,u-M1)
      end if
      end if
      end do
      end do
...
# int(A_blocks-1) es el numero de fotogramas totales en el archivo de datos
  ...
  14     format(3x,I4,2x,'O',3x,f10.5,2x,f10.5,2x,f10.5,4x,I3,3x,I4)
      ...
      real*8,dimension(1,500,500)::mat
      integer::i,j,u,nx,ny
